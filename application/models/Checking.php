...
class Application_Model_Checking extends Zend_Db_Table_Abstract {
    ...
    /*
     * use for add checking detail 
     */

    public function addChecking() {
        $data = array(
            'name' => $this->_cname,
    ...
    /*
     * get details for checking list view
     */

    public function getCheckings() {

        $select = $this->select()
                ->from(array('a' => 'checking'), array('name', 'id'))
                ->where('status = ?', 1)
                ->order('a.id')
    ...
    /*
     * get checking details for checking edit
     */

    public function getChecking() {
        $select = $this->select()
                ->from(array('a' => 'checking'), array('id', 'name', 'payment', 'description', 'create_by'))
                ->where('a.id =?', $this->_id)
                ->setIntegrityCheck(false);
    ...
    /*
     * use for  checking detail update
     */
    ...
    /*
     * use for delete a checking record
     */

    public function deleteChecking() {
        $data = array(
            'status' => $this->_status
        );
        $where = $this->getAdapter()->quoteInto('id = ?', $this->_id);
    ...
    public function checkName() {
        ...
        return $validator->isValid($this->_cname);
    }
